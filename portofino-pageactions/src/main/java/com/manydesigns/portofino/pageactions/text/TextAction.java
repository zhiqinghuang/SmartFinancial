...
public class TextAction extends AbstractPageAction {
	...
	public static final Logger logger = LoggerFactory.getLogger(TextAction.class);

	public Resolution preparePage() {
		textConfiguration = (TextConfiguration) pageInstance.getConfiguration();
		if (!pageInstance.getParameters().isEmpty()) {
			if (pageInstance.getParameters().size() == 1 && SecurityLogic.hasPermissions(portofinoConfiguration, pageInstance, SecurityUtils.getSubject(), AccessLevel.EDIT)) {
	...
	@Override
	@Buttons({ @Button(list = "configuration", key = "cancel", order = 99), @Button(list = "edit-content", key = "cancel", order = 99) })
	...
	protected String restoreAttachmentUrls(String content) {
		...
		while (matcher.find()) {
			...
			String hrefAttribute = (matcher.groupCount() >= 3 && matcher.group(3) != null) ? matcher.group(3) : "src";

			sb.append(content.substring(lastEnd, matcher.start())).append(hrefAttribute).append("=\"").append(StringEscapeUtils.escapeHtml(generateViewAttachmentUrl(attachmentId))).append("\"");
	...
	@Button(list = "pageHeaderButtons", titleKey = "edit", order = 2, icon = Button.ICON_EDIT, group = "pageHeaderButtons")
	...
	@Button(list = "pageHeaderButtons", titleKey = "configure", order = 1, icon = Button.ICON_WRENCH, group = "pageHeaderButtons")
	...
	protected void commonUploadAttachment() throws IOException {
		...
		File dataFile = RandomUtil.getCodeFile(pageInstance.getDirectory(), ATTACHMENT_FILE_NAME_PATTERN, attachmentId);
		...
		Attachment attachment = TextLogic.createAttachment(textConfiguration, attachmentId, upload.getFileName(), upload.getContentType(), upload.getSize());
		attachment.setDownloadable(uploadDownloadable);
		viewAttachmentUrl = generateViewAttachmentUrl(attachmentId);
	...
	protected String generateViewAttachmentUrl(String attachmentId) {
		return String.format("%s?viewAttachment=&id=%s", Util.getAbsoluteUrl(context.getActionPath()), attachmentId);
	}
	...
	protected Resolution streamAttachment(boolean isAttachment) {
		// find the attachment
		Attachment attachment = TextLogic.findAttachmentById(textConfiguration, id);
		...
		try {
			...
			// Suggerisce al browser di usare la risorsa che ha in cache invece
			// di riscaricarla - serve ancora?
			...
			StreamingResolution resolution = new StreamingResolution(attachment.getContentType(), is).setLength(attachment.getSize()).setFilename(attachment.getFilename()).setAttachment(isAttachment).setLastModified(file.lastModified());
			return resolution;
		} catch (IOException e) {
	...
	@Button(list = "pageHeaderButtons", titleKey = "manage.attachments", order = 3, icon = Button.ICON_PICTURE, group = "pageHeaderButtons")
	@RequiresPermissions(level = AccessLevel.VIEW, permissions = { PERMISSION_EDIT })
	...
	public Resolution deleteAttachments() {
		if (selection == null || selection.length == 0) {
			...
			for (String code : selection) {
				Attachment attachment = TextLogic.deleteAttachmentByCode(textConfiguration, code);
				if (attachment == null) {
					logger.warn("Ignoring non-existing attachment with code: {}", code);
					continue;
				}
				File file = RandomUtil.getCodeFile(pageInstance.getDirectory(), ATTACHMENT_FILE_NAME_PATTERN, attachment.getId());
			...
			if (counter == 1) {
				...
				SessionMessages.addInfoMessage(ElementsThreadLocals.getText("text.attachment.nDeleted", counter));
			}
		}
		return new RedirectResolution(context.getActionPath()).addParameter("manageAttachments").addParameter("returnUrl", returnUrl);
	}
