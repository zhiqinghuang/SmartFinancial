...
public class SecurityLogic {
	...
	public static boolean hasPermissions(Configuration conf, PageInstance instance, Subject subject, AccessLevel accessLevel, String... permissions) {
	...
	public static Permissions calculateActualPermissions(Permissions basePermissions, List<Page> pages) {
		...
		if (pages.size() > 0) {
			...
			Map<String, Set<String>> lastPermissions = lastPage.getPermissions().getActualPermissions();
	...
	public static boolean hasPermissions(Configuration conf, Permissions configuration, org.apache.shiro.mgt.SecurityManager securityManager, PrincipalCollection principals, AccessLevel level, String... permissions) {
		if (principals != null) {
			PagePermission pagePermission = new PagePermission(configuration, level, permissions);
	...
	public static boolean isAdministrator(ServletRequest request) {
		...
		Configuration conf = (Configuration) servletContext.getAttribute(BaseModule.PORTOFINO_CONFIGURATION);
	...
	public static boolean isAllowed(HttpServletRequest request, Dispatch dispatch, ActionBean actionBean, Method handler) {
		Subject subject = SecurityUtils.getSubject();
		...
		if (isNotAdmin) {
			...
			if (dispatch != null) {
				...
				allowed = hasPermissions(configuration, permissions, subject, handler, actionBean.getClass());
			}
			if (!allowed) {
