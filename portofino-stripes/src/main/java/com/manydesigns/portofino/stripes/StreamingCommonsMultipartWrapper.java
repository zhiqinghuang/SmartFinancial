...
/**
 * MultipartWrapper implementation that uses the streaming API of Commons
 * Fileupload, avoiding the use of files.
 *
...
public class StreamingCommonsMultipartWrapper implements MultipartWrapper {
	...
	public FileBean getFileParameterValue(String name) {
		...
		if (item == null || ((item.fileName == null || item.fileName.length() == 0) && item.size == 0)) {
			...
			// Attempt to ensure the file name is just the basename with no path
			// included
