...
public class SelectSearchField extends AbstractSearchField {
		...
		if (values == null) {
			...
			for (int i = 0; i < values.length; i++) {
				if (!StringUtils.isEmpty((String) values[i])) {
					castedValues[i] = OgnlUtils.convertValueQuietly(values[i], accessor.getType());
				}
			}
	...
	public void toXhtml(@NotNull XhtmlBuffer xb) {
		...
		case MULTIPLESELECT:
			...
			break;
		default:
			throw new IllegalStateException("Unknown display mode: " + displayMode.name());
		}
		xb.closeElement("div");
	}
	...
	public String composeDropDownJs() {
		...
		sb.append(MessageFormat.format("$(''#{0}'').change(" + "function() '{'" + "updateSelectOptions(''{1}'', {2}, ''jsonSelectFieldSearchOptions''", StringEscapeUtils.escapeJavaScript(id), StringEscapeUtils.escapeJavaScript(selectionModel.getName()), selectionModelIndex + 1));
		appendIds(sb);
		sb.append(");});");
		return sb.toString();
	}
	...
	public String composeAutocompleteJs() {
		StringBuilder sb = new StringBuilder();
		sb.append(MessageFormat.format("setupAutocomplete(''#{0}'', ''{1}'', {2}, ''jsonAutocompleteSearchOptions''", StringEscapeUtils.escapeJavaScript(autocompleteId), StringEscapeUtils.escapeJavaScript(selectionModel.getName()), selectionModelIndex));
		appendIds(sb);
		sb.append(");");
		return sb.toString();
	}
	...
	public void appendIds(StringBuilder sb) {
		...
		while (currentField != null) {
			sb.append(MessageFormat.format(", ''#{0}''", StringEscapeUtils.escapeJavaScript(currentField.getId())));
