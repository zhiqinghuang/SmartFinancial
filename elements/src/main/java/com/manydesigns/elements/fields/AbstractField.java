...
public abstract class AbstractField implements Field {
	...
	public AbstractField(@NotNull PropertyAccessor accessor, @NotNull Mode mode, @Nullable String prefix) {
		...
		Enabled enabledAnnotation = accessor.getAnnotation(Enabled.class);
		if (enabledAnnotation != null) {
			enabled = enabledAnnotation.value();
			logger.debug("Enabled annotation present with value: {}", enabled);
		}

		Insertable insertableAnnotation = accessor.getAnnotation(Insertable.class);
		if (insertableAnnotation != null) {
			insertable = insertableAnnotation.value();
			logger.debug("Insertable annotation present with value: {}", insertable);
		}

		Updatable updatableAnnotation = accessor.getAnnotation(Updatable.class);
		if (updatableAnnotation != null) {
			updatable = updatableAnnotation.value();
			logger.debug("Updatable annotation present with value: {}", updatable);
		}
	...
	/**
	 * Writes the HTML before a visible input field. The HTML includes the label
	 * and any necessary layout tags.
	 * 
	 * @param xb
	...
	/**
	 * Writes the HTML after a visible input field. The HTML closes any
	 * necessary layout tags opened by openVisibleField().
	 * 
	 * @param xb
	 *            the buffer to write to.
	 */
