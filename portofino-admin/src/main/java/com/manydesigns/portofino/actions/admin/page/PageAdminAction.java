...
public class PageAdminAction extends AbstractPageAction {
	...
	protected static final String[][] NEW_PAGE_SETUP_FIELDS = { { "actionClassName", "fragment", "title", "description", "insertPositionName" } };
	...
	public Resolution prepare() {
		...
		if (!SecurityLogic.hasPermissions(portofinoConfiguration, pageInstance, SecurityUtils.getSubject(), AccessLevel.EDIT)) {
	...
	@Buttons({ @Button(list = "page-children-edit", key = "cancel", order = 99), @Button(list = "page-permissions-edit", key = "cancel", order = 99), @Button(list = "page-create", key = "cancel", order = 99) })
	...
	protected Resolution copyPage(String destinationPagePath, String newName, boolean deleteOriginal) {
		...
		if (newParent != null) { // TODO vedi sopra
			...
			if (newParent.getParameters().isEmpty()) {
				return new RedirectResolution(destinationPagePath + (destinationPagePath.endsWith("/") ? "" : "/") + newName);
			} else {
				// Detail
	...
	private void prepareNewPageForm() throws Exception {
		...
		boolean includeSiblingOption = dispatch.getPageInstancePath().length > 2 && SecurityLogic.hasPermissions(portofinoConfiguration, dispatch.getPageInstance(-2), subject, AccessLevel.EDIT);
		...
		// Assumiamo che l'utente abbia almeno i permessi per creare una pagina
		// figlia, altrimenti non sarebbe qui.
		// In ogni caso, la verifica sui permessi viene ripetuta alla creazione
		// vera e propria.
		...
		newPageForm = new FormBuilder(NewPage.class).configFields(NEW_PAGE_SETUP_FIELDS).configFieldSetNames("Page setup").configSelectionProvider(classSelectionProvider, "actionClassName").configSelectionProvider(insertPositionSelectionProvider, "insertPositionName").build();
	...
	protected void setupChildPages() {
		...
		if (PageActionLogic.supportsDetail(getPageInstance().getActionClass())) {
			...
			detailChildPagesForm = setupChildPagesForm(detailChildPages, detailDirectory, getPage().getDetailLayout(), "detail");
		}
	}
	...
	protected TableForm setupChildPagesForm(List<EditChildPage> childPages, File childrenDirectory, Layout layout, String prefix) {
		...
		childPagesForm = new TableFormBuilder(EditChildPage.class).configNRows(childPages.size()).configFields(getChildPagesFormFields()).configPrefix(prefix).build();
	...
	public Resolution pagePermissions() {
		if (!checkPermissionsOnTargetPage(getPageInstance(), AccessLevel.DEVELOP)) { // Altrimenti
																						// un
																						// utente
																						// può
																						// cambiare
	...
	public Resolution testUserPermissions() {
		...
		for (AccessLevel level : AccessLevel.values()) {
			if (level.isGreaterThanOrEqual(testedAccessLevel) && SecurityLogic.hasPermissions(portofinoConfiguration, permissions, securityManager, principalCollection, level)) {
		...
		if (supportedPermissions != null) {
			for (String permission : supportedPermissions) {
				boolean permitted = SecurityLogic.hasPermissions(portofinoConfiguration, permissions, securityManager, principalCollection, testedAccessLevel, permission);
				if (permitted) {
	...
	public Resolution updatePagePermissions() {
		...
		try {
			...
			return new RedirectResolution(HttpUtil.getRequestedPath(context.getRequest())).addParameter("pagePermissions").addParameter("originalPath", originalPath);
		} catch (Exception e) {
	...
	protected void buildMovePageForm() {
		...
		SelectionProvider pagesSelectionProvider = DispatcherLogic.createPagesSelectionProvider(pagesDir, true, true, pageInstance.getDirectory());
		moveForm = new FormBuilder(MovePage.class).configReflectiveFields().configSelectionProvider(pagesSelectionProvider, "destinationPagePath").build();
	}

	protected void buildCopyPageForm() {
		PageInstance pageInstance = dispatch.getLastPageInstance();
		SelectionProvider pagesSelectionProvider = DispatcherLogic.createPagesSelectionProvider(pagesDir, true, true, pageInstance.getDirectory());
		copyForm = new FormBuilder(CopyPage.class).configReflectiveFields().configSelectionProvider(pagesSelectionProvider, "destinationPagePath").build();
	}
