...
public class TablesAction extends AbstractActionBean {
			...
			try {
				...
				for (Table otherTable : table.getSchema().getTables()) {
					for (ForeignKey fk : otherTable.getForeignKeys()) {
						if (fk.getFromTable().equals(table) || (!fk.getFromTable().equals(table) && fk.getToTable().equals(table))) {
							for (Reference ref : fk.getReferences()) {
								...
								if (fromColumn.getActualJavaType() != toColumn.getActualJavaType()) {
									SessionMessages.addWarningMessage(ElementsThreadLocals.getText("detected.type.mismatch.between.column._.and.column._", fromColumn.getQualifiedName(), fromColumn.getActualJavaType().getName(), toColumn.getQualifiedName(), toColumn.getActualJavaType().getName(), fk.getName()));
								}
							}
						}
					}
				}
				SessionMessages.consumeWarningMessages(); // Clear skipped
															// columns warnings
				// setupTableForm(Mode.EDIT); //Recalculate entity name
				// setupColumnsForm(Mode.EDIT); //Reflect the new order of the
				// columns
			} catch (Exception e) {
				logger.error("Could not save model", e);
				SessionMessages.addErrorMessage(e.toString());
			}
			return new RedirectResolution(TablesAction.class, "editTable").addParameter("databaseName", databaseName).addParameter("schemaName", schemaName).addParameter("tableName", tableName);
		} else {
			return new ForwardResolution("/m/admin/tables/edit-table.jsp");
		}
	}
	...
	public Resolution saveColumn() {
				...
				for (Table otherTable : table.getSchema().getTables()) {
					for (ForeignKey fk : otherTable.getForeignKeys()) {
						for (Reference ref : fk.getReferences()) {
							Column fromColumn = ref.getActualFromColumn();
							Column toColumn = ref.getActualToColumn();
							if ((fromColumn.equals(column) || toColumn.equals(column)) && fromColumn.getActualJavaType() != toColumn.getActualJavaType()) {
								Column otherColumn;
								...
								SessionMessages.addWarningMessage(ElementsThreadLocals.getText("detected.type.mismatch.with.column._", otherColumn.getQualifiedName(), otherColumn.getActualJavaType().getName(), fk.getName()));
							}
						}
					}
				}
		...
		return new RedirectResolution(TablesAction.class, "editColumn").addParameter("databaseName", databaseName).addParameter("schemaName", schemaName).addParameter("tableName", tableName).addParameter("columnName", columnName);
	}

	@Buttons({ @Button(key = "cancel", list = "column-edit", order = 2), @Button(key = "cancel", list = "table-short-name", order = 2), @Button(key = "cancel", list = "table-selection-provider", order = 3) })
	public Resolution returnToTable() {
		RedirectResolution resolution = new RedirectResolution(BASE_ACTION_PATH + "/" + databaseName + "/" + schemaName + "/" + tableName);
	...
	public Resolution editSelectionProvider() {
		table = findTable();
		databaseSelectionProvider = (DatabaseSelectionProvider) DatabaseLogic.findSelectionProviderByName(table, selectionProviderName);
		DatabaseSelectionProviderForm databaseSelectionProviderForm = setupDbSelectionProviderForm(Mode.CREATE);
		return doEditSelectionProvider(databaseSelectionProviderForm);
	}
	...
	protected DatabaseSelectionProviderForm setupDbSelectionProviderForm(Mode mode) {
		SelectionProvider databaseChooser = SelectionProviderLogic.createSelectionProvider("database", persistence.getModel().getDatabases(), Database.class, null, new String[] { "databaseName" });
		dbSelectionProviderForm = new FormBuilder(DatabaseSelectionProviderForm.class).configFields("name", "toDatabase", "hql", "sql", "columns").configSelectionProvider(databaseChooser, "toDatabase").configMode(mode).build();
		DatabaseSelectionProviderForm databaseSelectionProviderForm = new DatabaseSelectionProviderForm(databaseSelectionProvider);
	...
	public Resolution saveSelectionProvider() {
		...
		if (dbSelectionProviderForm.validate()) {
			...
			if ((!StringUtils.isEmpty(databaseSelectionProviderForm.getSql()) && !StringUtils.isEmpty(databaseSelectionProviderForm.getHql())) || (StringUtils.isEmpty(databaseSelectionProviderForm.getSql()) && StringUtils.isEmpty(databaseSelectionProviderForm.getHql()))) {
				SessionMessages.addErrorMessage(ElementsThreadLocals.getText("please.fill.exactly.one.of.the.fields.hql.sql"));
				return doEditSelectionProvider(databaseSelectionProviderForm);
			}
			...
			for (String c : refCols) {
				...
				if (col == null) {
					SessionMessages.addErrorMessage(ElementsThreadLocals.getText("column._.not.found", c));
					return doEditSelectionProvider(databaseSelectionProviderForm);
				} else {
					columns.add(col);
				}
			...
			if (!StringUtils.equals(databaseSelectionProviderForm.getName(), selectionProviderName)) {
				...
				if (DatabaseLogic.findSelectionProviderByName(table, databaseSelectionProviderForm.getName()) != null) {
					String message = ElementsThreadLocals.getText("selection.provider._.already.exists", databaseSelectionProviderForm.getName());
	...
	protected com.manydesigns.portofino.actions.admin.tables.forms.TableForm setupTableForm(Mode mode) {
		table = findTable();
		tableForm = new FormBuilder(com.manydesigns.portofino.actions.admin.tables.forms.TableForm.class).configFields("entityName", "javaClass", "shortName", "hqlQuery").configMode(mode).build();
		com.manydesigns.portofino.actions.admin.tables.forms.TableForm tf = new com.manydesigns.portofino.actions.admin.tables.forms.TableForm(table);
	...
	protected void setupColumnsForm(Mode mode) {
		...
		for (Column column : table.getColumns()) {
			...
			if (cf != null) {
				...
				SessionMessages.addWarningMessage(ElementsThreadLocals.getText("skipped.column._.with.unknown.type._", column.getColumnName(), column.getColumnType(), column.getJdbcType()));
			}
		}
		...
		columnsTableForm = new TableFormBuilder(ColumnForm.class).configFields("columnName", "propertyName", "javaType", "type", "shortLength", "scale", "reallyNullable").configSelectionProvider(typesSP, "columnName", "type", "javaType").configNRows(decoratedColumns.size()).configMode(mode).build();
		...
		for (int i = 0; i < decoratedColumns.size(); i++) {
			...
			columnNameField.setHref(context.getRequest().getContextPath() + getActionPath() + "/" + column.getColumnName());
		}
	...
	protected ColumnForm setupColumnForm() {
		...
		columnForm = new FormBuilder(ColumnForm.class).configFieldSetNames("Properties", "Annotations").configFields(new String[] { "columnName", "propertyName", "javaType", "type", "length", "scale", "reallyNullable", "reallyAutoincrement", "inPk" }, getApplicableAnnotations(column.getActualJavaType())).configSelectionProvider(typesSP, "columnName", "type", "javaType").configSelectionProvider(stringFormatSP, "stringFormat").configSelectionProvider(typeOfContentSP, "typeOfContent").build();
	...
	public Table findTable() {
		Table table = DatabaseLogic.findTableByName(persistence.getModel(), databaseName, schemaName, tableName);
		if (table == null) {
