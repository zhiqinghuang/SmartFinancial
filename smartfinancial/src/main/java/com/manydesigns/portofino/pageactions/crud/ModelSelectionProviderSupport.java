...
public class ModelSelectionProviderSupport implements SelectionProviderSupport {
	public static final Logger logger = LoggerFactory.getLogger(ModelSelectionProviderSupport.class);
	...
	public void setup() {
		...
		for (SelectionProviderReference ref : crudAction.getCrudConfiguration().getSelectionProviders()) {
			...
			if (ref.isEnabled() && !added) {
				AbstractCrudAction.logger.warn("Selection provider {} not added; check whether the fields on which it is configured " + "overlap with some other selection provider", ref);
			}
		}

		// Remove disabled selection providers and mark them as configured to
		// avoid re-adding them
	...
	protected boolean setupSelectionProvider(@Nullable SelectionProviderReference ref, DatabaseSelectionProvider current, Set<String> configuredSPs) {
		...
		if (ref == null || ref.isEnabled()) {
			...
			CrudSelectionProvider crudSelectionProvider = new CrudSelectionProvider(selectionProvider, fieldNames, newHref, newText);
			...
			CrudSelectionProvider crudSelectionProvider = new CrudSelectionProvider(null, fieldNames, null, null);
			crudSelectionProviders.add(crudSelectionProvider);
			return false;
		}
	}
	...
	protected SelectionProvider createSelectionProvider(DatabaseSelectionProvider current, String[] fieldNames, Class[] fieldTypes, DisplayMode dm, SearchDisplayMode sdm, String newHref, String newText) {
		...
		if (!anyActiveProperty) {
			// Dummy
			selectionProvider = SelectionProviderLogic.createSelectionProvider(current.getName(), 0, new Class[0], Collections.<Object[]> emptyList());
		} else {
	...
	public void disableSelectionProvider(List<String> key) {
		// TODO this is a shortcut: takes the first available selection provider
		// and disables it
	...
	protected DefaultSelectionProvider createSelectionProvider(DatabaseSelectionProvider current, String[] fieldNames, Class[] fieldTypes, DisplayMode dm, SearchDisplayMode sdm) {
		...
		if (!StringUtils.isEmpty(sql)) {
			...
			selectionProvider = SelectionProviderLogic.createSelectionProvider(name, objects, tableAccessor.getKeyProperties(), textFormats);
			selectionProvider.setDisplayMode(dm);
			selectionProvider.setSearchDisplayMode(sdm);
