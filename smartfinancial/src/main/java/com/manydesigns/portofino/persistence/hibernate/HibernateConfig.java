...
public class HibernateConfig {
	...
	public HibernateConfig(ConnectionProvider connectionProvider, org.apache.commons.configuration.Configuration portofinoConfiguration) {
	...
	protected void setupConfigurationProperties(Configuration configuration) {
		configuration.setProperty("hibernate.current_session_context_class", "org.hibernate.context.internal.ThreadLocalSessionContext").setProperty("org.hibernate.hql.ast.AST", "true").setProperty("hibernate.globally_quoted_identifiers", "false");
		// mettendo la modalità dynamic map, non funzionano le entità mappate su
		// bean.
		// configuration.setProperty("hibernate.default_entity_mode",
		// "dynamic-map");
	}

	protected void setupConnection(Configuration configuration) {
		if (!connectionProvider.isHibernateDialectAutodetected()) {
			configuration.setProperty("hibernate.dialect", connectionProvider.getActualHibernateDialectName());
		}
	...
	protected RootClass createTableMapping(Mappings mappings, com.manydesigns.portofino.model.database.Table aTable) {
		...
		Table tab = mappings.addTable(quoteIdentifier(aTable.getSchemaName()), null, quoteIdentifier(aTable.getTableName()), null, false);
		...
		if (columnPKList.size() > 1) {
			createPKComposite(mappings, aTable, aTable.getPrimaryKey().getPrimaryKeyName(), clazz, tab, columnPKList);
		} else {
	...
	protected Column createColumn(Mappings mappings, Table tab, com.manydesigns.portofino.model.database.Column column) {
	...
	protected void createPKComposite(Mappings mappings, com.manydesigns.portofino.model.database.Table mdTable, String pkName, RootClass clazz, Table tab, List<com.manydesigns.portofino.model.database.Column> columnPKList) {
		...
		if (!component.isDynamic()) {
			component.setComponentClassName(mdTable.getJavaClass()); // TODO
																		// verificare
																		// se
																		// non
																		// si
		...
		for (com.manydesigns.portofino.model.database.Column column : columnPKList) {
			...
			if (col != null) {
				...
				// prop.setPropertyAccessorName("property"); interferisce con il
				// generator più sotto
				...
				/*
				 * String columnName = column.getColumnName(); PrimaryKeyColumn
				 * pkCol =
				 * mdTable.getPrimaryKey().findPrimaryKeyColumnByName(columnName
				 * ); if(pkCol == null) {
				 * logger.error("Column without corresponding PrimaryKeyColumn: {}"
				 * , columnName); hasErrors = true; continue; } Generator
				 * generator = pkCol.getGenerator();
				 * setPKColumnGenerator(mappings, clazz, tab, column, value,
				 * generator);
				 */
			}
		}
		if (hasErrors) {
			// TODO PAOLO: se la PK non e' buona, tutta la tabella dovrebbe
			// saltare
	...
	protected void createPKSingle(Mappings mappings, com.manydesigns.portofino.model.database.Table mdTable, String pkName, RootClass clazz, Table tab, List<com.manydesigns.portofino.model.database.Column> columnPKList) {
		...
		// Make the defaults explicit. See section 5.1.4.5. Assigned identifiers
	...
	protected void setPKColumnGenerator(Mappings mappings, RootClass clazz, Table tab, com.manydesigns.portofino.model.database.Column column, SimpleValue id, Generator generator) {
		if (column.isAutoincrement()) {
			...
			if (generator instanceof SequenceGenerator) {
				...
				manageTableGenerator(mappings, tab, id, (com.manydesigns.portofino.model.database.TableGenerator) generator);
	...
	private void manageSequenceGenerator(Mappings mappings, Table tab, SimpleValue id, SequenceGenerator generator) {
		...
		Properties params = new Properties();
		params.put(PersistentIdentifierGenerator.IDENTIFIER_NORMALIZER, mappings.getObjectNameNormalizer());
		params.put(SequenceStyleGenerator.SEQUENCE_PARAM, quoteIdentifier(generator.getName()));
		params.setProperty(SequenceStyleGenerator.SCHEMA, quoteIdentifier(tab.getSchema()));
		id.setIdentifierGeneratorProperties(params);
		id.setNullValue(null);
	}

	private void manageTableGenerator(Mappings mappings, Table tab, SimpleValue id, com.manydesigns.portofino.model.database.TableGenerator generator) {
		id.setIdentifierGeneratorStrategy("enhanced-table");
		Properties params = new Properties();
		params.put(TableGenerator.TABLE, tab);
		params.put(TableGenerator.TABLE_PARAM, quoteIdentifier(generator.getTable()));
		params.put(PersistentIdentifierGenerator.IDENTIFIER_NORMALIZER, mappings.getObjectNameNormalizer());
		...
		params.setProperty(TableGenerator.SCHEMA, quoteIdentifier(tab.getSchema()));
		id.setIdentifierGeneratorProperties(params);
		id.setNullValue(null);
	}
	...
	protected void createO2M(Configuration config, Mappings mappings, ForeignKey relationship) {
		...
		if (oneMDTable.getJavaClass() != null) {
			try {
				...
				if (!found) {
					logger.warn("Property '{}' not found, skipping relationship {}", relationship.getActualManyPropertyName(), relationship.getQualifiedName());
					return;
				}
			} catch (Exception e) {
		...
		if (clazzOne == null) {
			logger.error("Cannot find table '{}' as 'one' side of foreign key '{}'. Skipping relationship.", oneMDQualifiedTableName, relationship.getName());
			return;
		}

		PersistentClass clazzMany = config.getClassMapping(manyMDQualifiedTableName);
		if (clazzMany == null) {
			logger.error("Cannot find table '{}' as 'many' side of foreign key '{}'. Skipping relationship.", manyMDQualifiedTableName, relationship.getName());
			return;
		}
		...
		if (refs.size() > 1) {
			dv = createFKComposite(mappings, relationship, manyMDTable, clazzOne, clazzMany, set, tableMany, tableOne, oneColumns, manyColumns);
	...
	private DependantValue createFKSingle(Mappings mappings, PersistentClass clazzOne, PersistentClass clazzMany, Table tableOne, List<Column> oneColumns, List<Column> manyColumns, List<Reference> refs) {
		DependantValue dv;
		Property refProp;
	...
	protected void createM2O(Configuration config, Mappings mappings, ForeignKey relationship) {
		...
		Set<com.manydesigns.portofino.model.database.Column> referencedColumns = new HashSet<com.manydesigns.portofino.model.database.Column>();
		for (Reference ref : relationship.getReferences()) {
			...
			// essa ha uno uniqueIdentifier generato al momento
			// dell'associazione alla tabella;
			// questo viene utilizzato per disambiguare l'alias della colonna
			// nelle query
			// SQL generate da Hibernate.
			col = manyClass.getTable().getColumn(col);
			if (col == null) {
				logger.error("Column not found in 'many' entity {}: {}, " + "skipping relationship", manyClass.getEntityName(), fromColumn.getColumnName());
				return;
			}
			m2o.addColumn(col);
			// Is the relationship a reference to the primary key or to a unique
			// key?
			referencedColumns.add(ref.getActualToColumn());
		}
		List<?> pkColumns = oneMDTable.getPrimaryKey().getColumns();
		boolean referenceToPrimaryKey = pkColumns.containsAll(referencedColumns) && pkColumns.size() == referencedColumns.size();
		m2o.setReferenceToPrimaryKey(referenceToPrimaryKey);
		if (!referenceToPrimaryKey) {
			logger.debug("Foreign key {} is not a reference to the primary key. Creating synthetic property.", relationship.getName());
			try {
				addSyntheticProperty(mappings, m2o, relationship, oneMDTable, oneMDQualifiedTableName);
			} catch (Exception e) {
				logger.error("Unsupported reference to columns outside the primary key, skipping relationship: " + relationship.getName(), e);
			}
		}
	...
	/**
	 * (Adapted from Hibernate) Create a synthetic property to refer to
	 * including an embedded component value containing all the properties
	 * mapped to the referenced columns. We need to shallow copy those
	 * properties to mark them as non insertable / non updatable.
	 */
	protected void addSyntheticProperty(Mappings mappings, ManyToOne m2o, ForeignKey relationship, com.manydesigns.portofino.model.database.Table oneMDTable, String oneMDQualifiedTableName) {
		...
		// String colToPropertyName =
		// reference.getActualToColumn().getActualPropertyName();
		// refProp = getRefProperty(clazzOne, colToPropertyName);
		...
		mappings.addPropertyReferencedAssociation(referencedClass.getEntityName(), firstPropertyName, syntheticPropertyName);
	}
	...
	public boolean setHibernateType(@Nullable SimpleValue value, com.manydesigns.portofino.model.database.Column column, Class javaType, final int jdbcType) {
		String typeName;
