...
public abstract class LoginAction extends AbstractActionBean {
	...
	public Resolution forgotPassword2() {
		...
		try {
			...
			if (user != null) {
				...
				String from = portofinoConfiguration.getString(PortofinoProperties.MAIL_FROM, "example@example.com");
				sendForgotPasswordEmail(from, email, ElementsThreadLocals.getText("password.reset.confirmation.required"), body);
			}
	...
	public Resolution signUp2() {
		...
		try {
			...
			String from = portofinoConfiguration.getString(PortofinoProperties.MAIL_FROM, "example@example.com");
			sendSignupConfirmationEmail(from, email, ElementsThreadLocals.getText("confirm.signup"), body);
			SessionMessages.addInfoMessage(ElementsThreadLocals.getText("check.your.mailbox.and.follow.the.instructions"));
			return authenticate();
	...
	protected boolean validateCaptcha() {
		...
		boolean valid = StringUtils.equalsIgnoreCase(request.getParameter("captchaText"), (String) session.getAttribute(CAPTCHA_SESSION_ATTRIBUTE));
	...
	public Resolution changePassword2() throws Exception {
		...
		try {
			...
			if (subject.isRemembered()) {
				UsernamePasswordToken usernamePasswordToken = new UsernamePasswordToken(getRememberedUserName(principal), newPassword);
				usernamePasswordToken.setRememberMe(true);
				try {
					subject.login(usernamePasswordToken);
				} catch (Exception e) {
					logger.warn("User {} changed password but could not be subsequently authenticated", portofinoRealm.getUserId(principal));
				}
			}
	...
	protected Resolution redirectToReturnUrl(String returnUrl) {
		if (StringUtils.isEmpty(returnUrl)) {
			...
			try {
				...
				// Ok, if it is not a full URL there's no risk of XSS attacks
				// with returnUrl=http://www.evil.com/hack
			}
