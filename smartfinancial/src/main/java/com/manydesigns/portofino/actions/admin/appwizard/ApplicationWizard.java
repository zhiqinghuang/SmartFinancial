...
public class ApplicationWizard extends AbstractPageAction {
	...
	public final static String[] JDBC_CP_FIELDS = { "databaseName", "driver", "url", "username", "password" };
	...
	protected void buildCPForms() {
		...
		for (Database db : persistence.getModel().getDatabases()) {
			...
			if (!ConnectionProvider.STATUS_ERROR.equals(cp.getStatus())) {
				connectionProviderSP.appendRow(db.getDatabaseName(), db.getDatabaseName() + " (" + cp.getDatabasePlatform().getDescription() + ")", true);
			}
		}

		ClassAccessor classAccessor = JavaClassAccessor.getClassAccessor(ApplicationWizard.class);
		try {
			connectionProviderField = new SelectField(classAccessor.getProperty("connectionProviderName"), connectionProviderSP, Mode.EDIT, null);
		...
		jndiCPForm = new FormBuilder(ConnectionProviderForm.class).configFields(JNDI_CP_FIELDS).configPrefix("jndi").configMode(Mode.CREATE).build();
		...
		jdbcCPForm = new FormBuilder(ConnectionProviderForm.class).configFields(JDBC_CP_FIELDS).configPrefix("jdbc").configMode(Mode.CREATE).configSelectionProvider(driverSelectionProvider, "driver").build();
	...
	public Resolution configureConnectionProvider() {
		...
		if (!isNewConnectionProvider()) {
			connectionProvider = DatabaseLogic.findDatabaseByName(persistence.getModel(), connectionProviderName).getConnectionProvider();
		...
		if (JDBC.equals(connectionProviderType)) {
			...
			// Fill with dummy values so the form overwrites them (and doesn't
			// try to write on the Configuration which
		...
		if (connectionProviderForm.validate()) {
			...
			Database existingDatabase = DatabaseLogic.findDatabaseByName(persistence.getModel(), edit.getDatabaseName());
	...
	protected void configureEditSchemas() throws Exception {
		...
		schemasForm = new TableFormBuilder(SelectableSchema.class).configFields("selected", "schemaName").configMode(Mode.EDIT).configNRows(selectableSchemas.size()).configPrefix("schemas_").build();
		schemasForm.readFromObject(selectableSchemas);
		// Handle back
		schemasForm.readFromRequest(context.getRequest());
	}

	@Buttons({ @Button(list = "select-schemas", key = "next>>", order = 2, type = Button.TYPE_PRIMARY), @Button(list = "select-user-fields", key = "<<previous", order = 1) })
	...
	protected void updateModelFailed(Exception e) {
		...
		SessionMessages.addErrorMessage(ElementsThreadLocals.getText("could.not.save.model._", ExceptionUtils.getRootCauseMessage(e)));
		if (isNewConnectionProvider()) {
	...
	public Resolution afterSelectSchemas() {
		...
		rootsForm = new TableFormBuilder(SelectableRoot.class).configFields("selected", "tableName").configMode(Mode.EDIT).configNRows(selectableRoots.size()).configPrefix("roots_").build();
		...
		try {
			...
			for (Table table : allTables) {
				...
				selectionProvider.appendRow(table.getQualifiedName(), tableName, true);
			}
	...
	protected void createUserManagementSetupForm() {
		...
		for (Column column : userTable.getColumns()) {
			userSelectionProvider.appendRow(column.getActualPropertyName(), column.getActualPropertyName(), true);
		}

		DefaultSelectionProvider algoSelectionProvider = new DefaultSelectionProvider("");
		algoSelectionProvider.appendRow("plaintext:plaintext", ElementsThreadLocals.getText("plain.text"), true);
		algoSelectionProvider.appendRow("MD5:base64", ElementsThreadLocals.getText("md5.base64.encoded"), true);
		algoSelectionProvider.appendRow("MD5:hex", ElementsThreadLocals.getText("md5.hex.encoded"), true);
		algoSelectionProvider.appendRow("SHA-1:base64", ElementsThreadLocals.getText("sha1.base64.encoded.portofino3"), true);
		algoSelectionProvider.appendRow("SHA-1:hex", ElementsThreadLocals.getText("sha1.hex.encoded"), true);
		algoSelectionProvider.appendRow("SHA-256:base64", ElementsThreadLocals.getText("sha256.base64.encoded"), true);
		algoSelectionProvider.appendRow("SHA-256:hex", ElementsThreadLocals.getText("sha256.hex.encoded"), true);
		try {
			ClassAccessor classAccessor = JavaClassAccessor.getClassAccessor(getClass());
			Mode mode = Mode.CREATE;
			...
			if (groupTable != null && userGroupTable != null) {
				...
				for (Column column : groupTable.getColumns()) {
					groupSelectionProvider.appendRow(column.getActualPropertyName(), column.getActualPropertyName(), true);
				}
				...
				for (Column column : userGroupTable.getColumns()) {
					userGroupSelectionProvider.appendRow(column.getActualPropertyName(), column.getActualPropertyName(), true);
				}
	...
	protected void setupCalendarField() {
		ClassAccessor classAccessor = JavaClassAccessor.getClassAccessor(ApplicationWizard.class);
		try {
			generateCalendarField = new BooleanField(classAccessor.getProperty("generateCalendar"), Mode.EDIT);
	...
	protected List<Table> determineRoots(Multimap<Table, Reference> children, List<Table> allTables) {
		...
		for (Iterator<Table> it = roots.iterator(); it.hasNext();) {
			...
			boolean removed = false; // Did we already remove the table from the
										// list of roots?
			boolean selected = false; // Is the table selected as a root? Note
										// that selected => known
			boolean known = false; // Is the table in the list of selectable
									// roots?
			...
			if (!known) {
				SelectableRoot root = new SelectableRoot(table.getSchemaName() + "." + table.getTableName(), !removed);
	...
	public Resolution buildApplication() {
		selectTables();
		Database oldDatabase = DatabaseLogic.findDatabaseByName(persistence.getModel(), database.getDatabaseName());
		if (oldDatabase != null) {
	...
	protected void setupCalendar(List<ChildPage> childPages) throws Exception {
		...
		Color[] colors = { Color.RED, new Color(64, 128, 255), Color.CYAN.darker(), Color.GRAY, Color.GREEN.darker(), Color.ORANGE, Color.YELLOW.darker(), Color.MAGENTA.darker(), Color.PINK };
		int colorIndex = 0;
		for (Table table : allTables) {
			...
			if (!dateColumns.isEmpty()) {
				// ["Cal 1", "db1.schema1.table1", ["column1", "column2"],
				// Color.RED]
		...
		if (!calendarDefinitions.isEmpty()) {
			...
			if (dir.mkdirs()) {
				...
				SessionMessages.addWarningMessage(ElementsThreadLocals.getText("couldnt.create.directory", dir.getAbsolutePath()));
			}
		}
	}
	...
	protected void setupUserPages(List<ChildPage> childPages, Template template) throws Exception {
		...
		if (references != null) {
			for (Reference ref : references) {
				...
				Page page = createCrudPage(dir, fromTable, childQuery, childPages, template, bindings, title);
				if (page != null) {
	...
	protected Page createCrudPage(File dir, Table table, List<ChildPage> childPages, Template template) throws Exception {
	...
	protected Page createCrudPage(File dir, Table table, String query, List<ChildPage> childPages, Template template, Map<String, String> bindings, String title) throws Exception {
		if (dir.exists()) {
			SessionMessages.addWarningMessage(ElementsThreadLocals.getText("directory.exists.page.not.created._", dir.getAbsolutePath()));
			return null;
		} else if (dir.mkdirs()) {
			...
			if (!detailDir.isDirectory() && !detailDir.mkdir()) {
				...
				SessionMessages.addWarningMessage(ElementsThreadLocals.getText("couldnt.create.directory", detailDir.getAbsolutePath()));
			}
	...
	protected int setupColumn(Column column, CrudConfiguration configuration, int columnsInSummary, String linkToParentProperty) {
		...
		boolean enabled = !(linkToParentProperty != NO_LINK_TO_PARENT && column.getActualPropertyName().equals(linkToParentProperty)) && !isUnsupportedProperty(column);
		boolean propertyIsUserPassword = table.getQualifiedName().equals(userTableName) && column.getActualPropertyName().equals(userPasswordProperty);
		boolean inPk = DatabaseLogic.isInPk(column);
		boolean inFk = DatabaseLogic.isInFk(column);
		boolean inSummary = enabled && (inPk || columnsInSummary < maxColumnsInSummary) && !propertyIsUserPassword;
		...
		if (!propertyIsUserPassword && column.getActualJavaType() == String.class && (column.getLength() == null || column.getLength() > MULTILINE_THRESHOLD) && isNewConnectionProvider()) {
	...
	protected void detectBooleanColumn(Table table, Column column) {
		...
		if (column.getJdbcType() == Types.INTEGER || column.getJdbcType() == Types.DECIMAL || column.getJdbcType() == Types.NUMERIC) {
			...
			try {
				...
				liquibase.database.Database implementation = DatabaseFactory.getInstance().findCorrectDatabaseImplementation(new JdbcConnection(connection));
				String sql = "select count(" + implementation.escapeColumnName(null, null, null, column.getColumnName()) + ") " + "from " + implementation.escapeTableName(null, table.getSchemaName(), table.getTableName());
				...
				if (count == null || count < 10) {
					logger.info("Cannot determine if numeric column {} is boolean, count is {}", column.getQualifiedName(), count);
					return;
				}

				sql = "select distinct(" + implementation.escapeColumnName(null, null, null, column.getColumnName()) + ") " + "from " + implementation.escapeTableName(null, table.getSchemaName(), table.getTableName());
	...
	protected void detectLargeResultSet(Table table, CrudConfiguration configuration) {
		...
		try {
			...
			connection = connectionProvider.acquireConnection();
			liquibase.database.Database implementation = DatabaseFactory.getInstance().findCorrectDatabaseImplementation(new JdbcConnection(connection));
			String sql = "select count(*) from " + implementation.escapeTableName(null, table.getSchemaName(), table.getTableName());
			PreparedStatement statement = connection.prepareStatement(sql);
			setQueryTimeout(statement, 1);
			statement.setMaxRows(1);
			ResultSet rs = statement.executeQuery();
			if (rs.next()) {
				...
				if (count != null) {
					if (count > LARGE_RESULT_SET_THRESHOLD) {
						logger.info("Table " + table.getQualifiedName() + " currently has " + count + " rows, which is bigger than " + "the threshold (" + LARGE_RESULT_SET_THRESHOLD + ") for large result sets. It will be " + "marked as largeResultSet = true and no autodetection based on table data will be " + "attempted, in order to keep the processing time reasonable.");
						configuration.setLargeResultSet(true);
					} else {
						logger.info("Table " + table.getQualifiedName() + " currently has " + count + " rows, which is smaller than " + "the threshold (" + LARGE_RESULT_SET_THRESHOLD + ") for large result sets. It will be " + "analyzed normally.");
					}
				} else {
	...
	protected void createChildCrudPage(File dir, Template template, String parentName, Collection<Reference> references, Reference ref, ArrayList<ChildPage> pages) throws Exception {
		Column fromColumn = ref.getActualFromColumn();
		Table fromTable = fromColumn.getTable();
		String entityName = fromTable.getActualEntityName();
		...
		String childQuery = "from " + entityName + " where " + linkToParentProperty + " = %{#" + parentName + "." + parentProperty + "}" + " order by id desc";
