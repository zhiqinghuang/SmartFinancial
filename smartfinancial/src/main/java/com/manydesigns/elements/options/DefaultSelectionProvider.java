...
public class DefaultSelectionProvider implements SelectionProvider {
	...
	public final static Logger logger = LoggerFactory.getLogger(DefaultSelectionProvider.class);
	public static final String NON_WORD_CHARACTERS = " \t\n\f\r\\||!\"\u00ac\u00a3$\u201a\u00c7\u00a8%&/()='?^[]+*@#<>,;.:-_";

	public DefaultSelectionProvider(String name, int fieldCount, Row[] rows) {
		this(name, fieldCount, new ArrayList<Row>(Arrays.asList(rows)));
	}

	public DefaultSelectionProvider(String name, int fieldCount, List<Row> rows) {
		this.name = name;
		...
		private void validate() {
			...
			for (Row row : rows) {
				...
				for (int j = 0; j < fieldCount; j++) {
					...
					} else if (matching && value != null && value instanceof Object[] && ArrayUtils.contains((Object[]) value, cellValue)) {
						if (j > maxMatchingIndex) {
							maxMatchingIndex = j;
						}
					} else {
	...
	private boolean matchLabel(String cellLabel, String labelSearch2) {
		...
		String[] cellLabelArray = StringUtils.split(cellLabel, NON_WORD_CHARACTERS);
		String[] searchLabelArray = StringUtils.split(labelSearch2, NON_WORD_CHARACTERS);
