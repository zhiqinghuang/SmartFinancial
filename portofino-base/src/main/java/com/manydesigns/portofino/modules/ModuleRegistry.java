			...
			try { // Migrate
				while (installedVersion < migrationVersion) {
					...
					if (result > installedVersion) {
						...
						throw new RuntimeException("Migration returned version " + result + " while the installed one is " + installedVersion);
					}
				}
			} catch (Throwable e) {
				logger.error("Could not migrate module " + printModule(module) + " from version " + installedVersion, e);
				migrationOk = false;
			}

			if (migrationOk) { // Init (skip if installation or migration
								// failed)
				try {
