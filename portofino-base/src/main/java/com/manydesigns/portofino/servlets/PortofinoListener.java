...
public class PortofinoListener implements ServletContextListener, HttpSessionListener, ServletContextAttributeListener {
	...
	public void contextInitialized(ServletContextEvent servletContextEvent) {
		try {
			...
			ElementsThreadLocals.getOgnlContext().put(ElementsFilter.SERVLET_CONTEXT_OGNL_ATTRIBUTE, servletContextAttributeMap);
	...
	private void init(ServletContextEvent servletContextEvent) {
		...
		servletContext.setAttribute(BaseModule.ELEMENTS_CONFIGURATION, elementsConfiguration);
		...
		if (blobs == null || blobs.length == 0) { // Null if the directory does
													// not exist yet
		...
		String encoding = configuration.getString(PortofinoProperties.URL_ENCODING, PortofinoProperties.URL_ENCODING_DEFAULT);
	...
	protected GroovyScriptEngine createScriptEngine(File classpathFile) {
		...
		try {
			scriptEngine = new GroovyScriptEngine(new URL[] { classpathFile.toURI().toURL() }, getClass().getClassLoader());
		} catch (IOException e) {
			throw new Error(e);
		}
	...
	protected void loadConfiguration() throws ConfigurationException {
		...
		File localConfigurationFile = new File(applicationDirectory, "portofino-local.properties");
